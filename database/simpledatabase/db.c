...
// page 大小 4k 大部分系统以 4k 作为虚拟内存 page 大小
...
void *row_slot(Table *table, uint32_t row_num) 
{
  ...
  if (page == NULL) {
    // 如果 page 不存在就分配内存，因此 page 可以互相不紧挨着
    // row_slot(table, table->num_rows) 追加数据
    page = table->pages[page_num] = malloc(PAGE_SIZE);
  }
  uint32_t row_offset = row_num % ROWS_PRE_PAGE;
  uint32_t byte_offset = row_offset * ROW_SIZE;
  return page + byte_offset; // 返回 cursor
}
  ...
  while(1)
  {
    ...
    switch (prepare_statement(input_buffer, &statement)) {
      ...
      case (PREPARE_SYNTAX_ERROR):
        printf("Syntax error. Couldn't parse statement: %s\n", input_buffer->buffer);
        continue;
